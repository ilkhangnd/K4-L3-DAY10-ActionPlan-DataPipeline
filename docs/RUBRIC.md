...
| **6** | **Baseline Evaluation & Scoring** | **10** | `src/evaluation/testset.py`, `data/results/baseline_metrics.json`, `data/reports/phase1_report.md` | Sinh bộ testset 10 câu hỏi đa dạng qua 5 dạng nghiệp vụ (`summary`, `authors`, `date`, `category`, `multi_hop`). Đánh giá đo lường đầy đủ Hit Rate và Token F1 trên dữ liệu sạch. |
